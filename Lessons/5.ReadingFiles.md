...
2. **The `header`** - This argument refers to the column headers that may (`TRUE`) or (`FALSE`) **in the plain text file you are reading in**. 
...
3. **na.strings** - This argument refers to which strings should be treated as NA (missing values). It assumes all missing values are encoded by 'NA', but very often real world data contains other values such as, "N/A", ".", "", "missing" etc. In these situations, all these other missing data types can be coerced into 'NA' by na.strings.
...
```
4. **The `stringsAsFactors` argument** - This argument is set to FALSE by default (you can double check this by searching the Help tab for `read.table` or running `?read.table` in the console).
If `stringsAsFactors = TRUE`, it will coerce `character` columns in the data you are reading in to `factor` columns (i.e., coerce from `vector` to `factor`) in the resulting data frame. 
...
5. ** row.names** - This argument specifies which column should be read as row names.
   ...
   For example, if you have a table like this,
	...
	S2,22,91
	```
	and if you set ```row.names = 1```. It will read it in as;
